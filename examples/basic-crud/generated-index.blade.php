{{-- 
  Generated by Wink View Generator
  Command: php artisan wink:views:crud posts --framework=bootstrap --ajax --search --sorting --filtering
  
  This is an example of what gets generated for the index view.
  Features included: search, sorting, filtering, pagination, bulk actions, AJAX
--}}
            ...
            <div class="card shadow">
                ...
                <div class="card-body p-0">
                    ...
                    <div class="p-3 border-bottom bg-light">
                        <form method="GET" class="row g-3" aria-label="Search and filter posts">
                            <div class="col-md-4">
                                ...
                                <div id="search-help" class="form-text">Search by title, content, or excerpt</div>
                            </div>
                            ...
                            <div class="col-md-3 d-flex align-items-end">
                                <div class="btn-group w-100" role="group">
                                    ...
                                    <button type="button" 
                                            ...
                                            onclick="document.querySelector('form').reset(); location.href='{{ route('posts.index') }}';">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover mb-0" role="table" aria-label="Posts data table">
                                ...
                                @forelse($posts as $post)
                                    <tr>
                                        ...
                                        <td data-label="Status">
                                            <span class="badge bg-{{ $post->status_badge }}">
                                                <i class="bi bi-{{ $post->status_icon }} me-1"></i>
                                                {{ ucfirst($post->status) }}
                                            </span>
                                        </td>
                                        <td data-label="Created">
                                            <span title="{{ $post->created_at->format('M d, Y g:i A') }}">
                                                {{ $post->created_at->format('M d, Y') }}
                                            </span>
                                        </td>
                                        <td>
                                            <div class="btn-group" role="group" aria-label="Actions for {{ $post->title }}">
                                                @can('view', $post)
...
<div class="modal fade" id="bulkDeleteModal" tabindex="-1" aria-labelledby="bulkDeleteModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete <strong id="bulkDeleteCount"></strong> selected posts?</p>
                <p class="text-muted mb-0">This action cannot be undone.</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
...
function performDelete(id) {
    ...
    .then(data => {
        ...
        if (data.success) {
            showAlert('success', data.message || 'Post deleted successfully');
            setTimeout(() => location.reload(), 1000);
        } else {
            showAlert('danger', data.message || 'Error deleting post');
        }
    })
    .catch(error => {
        hideLoading(document.body);
...
document.getElementById('confirmBulkDeleteBtn')?.addEventListener('click', function() {
    ...
    .then(data => {
        ...
        if (data.success) {
            showAlert('success', data.message || `${ids.length} posts deleted successfully`);
            setTimeout(() => location.reload(), 1000);
        } else {
            showAlert('danger', data.message || 'Error deleting posts');
        }
    })
    .catch(error => {
        hideLoading(document.body);
        bootstrap.Modal.getInstance(document.getElementById('bulkDeleteModal')).hide();
        showAlert('danger', 'An error occurred while deleting the posts');
        console.error('Error:', error);
    });
});
...
function exportSelected() {
    ...
    form.method = 'POST';
    form.action = '{{ route('posts.export') }}';
